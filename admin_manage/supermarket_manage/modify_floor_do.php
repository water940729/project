...
//	print_r($_POST);
	...
	if(!empty($_POST["simple_color_value"])){
		$sql="update super_floorManage set name='{$floor_name}',weight={$weight},background='{$_POST["simple_color_value"]}' where id=$id";	
	}else{
		$sql="update super_floorManage set name='{$floor_name}',weight={$weight} where id=$id";	
	}
