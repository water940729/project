...
测试
	...
	$insert="insert into teambuy_goods(type_id,goodsname,mall,shop,keywords,price,original_price,introduction,img_url,start,end,num,description,extattribute1,extattribute2,extattribute3,role) 
	...
	$url="add_teambuy_goods.php";
	//echo $insert;
	if(mysql_query($insert)){
		$goods_id=mysql_insert_id();
		for($i=0;$i<count($_POST["pics"]);++$i){
			$insert_goods_pic="insert into teambuy_goods_pictures(goods_id,image_url,time) values('$goods_id','{$_POST["pics"][$i]}','$time')";
			//echo $insert_goods_pic."<br>";
			mysql_query($insert_goods_pic);
		}
		$content="添加了一种预售商品，商品信息：商品名".$goodsName;
		if(add_system_log($content)==1){
			$url="manage_teambuy_type.php?id=$type_id";
