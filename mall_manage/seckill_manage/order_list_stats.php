	...
	class order_List_Stats{
		...
		function getNum(){
			if(isset($this->status)){
				$sql1="select count(order_id) as total from seckill_orderlist where state=$this->status and $this->type=$this->id";
			}else{
		...
		function getResult($a,$b){			
			if(isset($this->status)){
				$sql1="select order_id,user_id,mall_id,shop_id,good_id,num,price,username,rec_name,address,phone,state,time from seckill_orderlist where state=$this->status and $this->type=$this->id order by order_id desc limit ".$a.",".$b;
			}else{
